...
def process(args):
    dataset_list = ["hts", "natsume", "pjs", "jsut", "kiritan", "ofuton_p_utagoe_db", "oniku_kurumi_utagoe_db"]
    ...
    for dataset in dataset_list:
        phone_dict_file = os.path.join(args.data_root_path, dataset, "data/phone_set.txt")
    ...
    for dataset in dataset_list:
        ...
        for split in ["train", "dev", "test"]:
            ...
            pitch_beat_root_path = os.path.join(db_root_path, "pitch_beat_extraction", split)
            ...
            for filename in filename_list:
                ...
                for i in range(len(alignment_id_old)):
                    ...
                    assert phone_combine_list[int(alignment_id[i])] == phone_dict_old[int(alignment_id_old[i])]
                ...
                cmd = "ln -s " + beat_path + " " + os.path.join(song_pitch_beat, dataset+"_"+filename[:-4]+ "_beats.npy")
                os.system(cmd)
                ...
                cmd = "ln -s " + pitch_path + " " + os.path.join(song_pitch_beat, dataset+"_"+filename[:-4]+ "_pitch.npy")
                os.system(cmd)
                ...
                cmd = "ln -s " + wav_path + " " + os.path.join(song_wav, dataset+"_"+filename[:-4]+ ".wav")
                os.system(cmd)
...
if __name__ == "__main__":
    ...
    parser.add_argument("data_root_path", type=str, help="path of public_dataset folder")
