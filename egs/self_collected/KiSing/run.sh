...
# Copyright 2021 RUC & Johns Hopkins University & Peking University(author: Shuai Guo, Jiatong Shi, Lei Li)
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
  ...
  echo "please download kiritan dataset from http://shijt.site/index.php/2021/03/15/kising-the-first-open-source-mandarin-singing-voice-synthesis-corpus/"
  echo "put segmented-singing.zip, segmented-midi.zip, segmented-label.zip under ${raw_data_dir}"
  unzip -o ${raw_data_dir}/segmented-singing.zip -d ${raw_data_dir}/segmented-wav
  unzip -o ${raw_data_dir}/segmented-midi.zip -d ${raw_data_dir}/segmented-midi
  unzip -o ${raw_data_dir}/segmented-label.zip -d ${raw_data_dir}
fi
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
  ...
  python local/prepare_data.py ${raw_data_dir}/segmented-wav/clean ${raw_data_dir}/segmented-label data \
    --sil pau 
