...
#sys.path.append("/Users/linhailan1/Desktop/林海斓/SVS/SVS_system")
import torch
...
class Encoder(nn.Module):
    ...
    Encoder Network
    """
    def __init__(self, phone_size, embed_size, nhead):
        """
        ...
        self.self_attn = module.MultiheadAttention(nhead, embed_size)
        
        self.fc_2 = nn.Linear(embed_size, embed_size)
    
    def forward(self, text_phone, src_mask=None, src_key_padding_mask=None):
        """
        ...
        att_src = self.fc_1(embedded_phone)
        
        att_out, att_weight = self.self_attn(att_src, att_src, att_src, unseen_mask=src_mask, src_lengths=src_key_padding_mask)

        out = self.fc_2(att_out)
        
        out = embedded_phone + out
...
def _test():
    ...
    encoder = Encoder(phone_size, embed_size, nhead)
